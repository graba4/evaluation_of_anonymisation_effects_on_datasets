...
if __name__ == '__main__':
    ...
    description_file = './titanic/out/{}/description.json'.format(mode)
    synthetic_data = './titanic/out/{}/sythetic_data.csv'.format(mode)
    ...
    candidate_keys = {'Fare': True, 'Survived': True}
    ...
    num_tuples_to_generate = 712  # Here 32561 is the same as input dataset, but it can be set to another number.

    describer = DataDescriber(histogram_bins='fd',
                              category_threshold=threshold_value,
                              null_values=na_values)
    describer.describe_dataset_in_independent_attribute_mode(input_data,
                                                            epsilon=eps,
